...
(async () => {
  ...
  try {
    ...
    const [fkRows] = await db.query(
      "SELECT CONSTRAINT_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='answers' AND REFERENCED_TABLE_NAME='questions'"
    );
    for (const r of fkRows) {
      await exec(
        `ALTER TABLE answers DROP FOREIGN KEY \`${r.CONSTRAINT_NAME}\``
      );
    }

    // 2. Remove legacy column questionid from questions (drop unique index first if still exists)
    // Check if column exists
    const [qCols] = await db.query(
      "SHOW COLUMNS FROM questions LIKE 'questionid'"
    );
    if (qCols.length) {
      // Drop index if any
      const [idx] = await db.query(
        "SHOW INDEX FROM questions WHERE Column_name='questionid'"
      );
      for (const i of idx) {
    ...
    if (hasAnswerId) {
      ...
      const [checkIdAfter] = await db.query(
        'SHOW COLUMNS FROM answers LIKE "id"'
      );
      if (!checkIdAfter.length) {
        await exec(
          "ALTER TABLE answers ADD COLUMN `id` INT NOT NULL AUTO_INCREMENT PRIMARY KEY FIRST"
        );
      } else {
        // ensure it's primary key
        const [pkInfo] = await db.query(
          "SHOW INDEX FROM answers WHERE Key_name='PRIMARY'"
        );
        if (!pkInfo.length) {
      ...
      const [aIdx] = await db.query(
        "SHOW INDEX FROM answers WHERE Column_name='answerid'"
      );
      for (const i of aIdx) {
    ...
    const [aQcol] = await db.query(
      "SHOW COLUMNS FROM answers LIKE 'questionid'"
    );
    if (aQcol.length) {
      const col = aQcol[0];
      if (!/int/i.test(col.Type)) {
        await exec(
          "ALTER TABLE answers MODIFY COLUMN `questionid` INT NOT NULL"
        );
      }
    } else {
      // If absent, maybe named question_id; rename for consistency with code expects questionid
      const [aQcol2] = await db.query(
        "SHOW COLUMNS FROM answers LIKE 'question_id'"
      );
      if (aQcol2.length) {
        await exec(
          "ALTER TABLE answers CHANGE COLUMN `question_id` `questionid` INT NOT NULL"
        );
      } else {
    ...
    await exec(
      "ALTER TABLE answers ADD CONSTRAINT `answers_question_fk` FOREIGN KEY (`questionid`) REFERENCES `questions`(`id`) ON DELETE CASCADE"
    );
    ...
    if (!timestampCols.includes("created_at")) {
      await exec(
        "ALTER TABLE answers ADD COLUMN `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP"
      );
    }
